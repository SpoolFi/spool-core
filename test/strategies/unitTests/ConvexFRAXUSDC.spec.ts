...
import {
    ...
    ConvexShared2poolStrategy__factory,
    IBaseStrategy,
    IERC20,
...
describe("Strategies Unit Test: Convex FRAXUSDC", () => {
    ...
    it("Should fail deploying strategy with underlying address 0", async () => {
        const ConvexSharedStrategy = new ConvexShared2poolStrategy__factory().connect(accounts.administrator);
        await expect(
        ...
        describe(`Asset: ${name}`, () => {
            ...
            describe(`Deployment: ${name}`, () => {
                it("Should deploy", async () => {
                    const convexStrat = await new ConvexShared2poolStrategy__factory()
            ...
            describe(`Functions: ${name}`, () => {
                ...
                beforeEach(async () => {
                    ...
                    const convexStrategyImpl = await new ConvexShared2poolStrategy__factory()
                        .connect(accounts.administrator)
                        .deploy(
                            mainnetConst.convex.Booster.address,
                            mainnetConst.convex._fraxusdc.boosterPoolId,
                    ...
                    convexContract = ConvexShared2poolStrategy__factory.connect(
                        convexStrategyProxy.address,
                        accounts.administrator
                    );
