...
import {
    ConvexBoosterContractHelperV2__factory,
    ConvexShared2poolStrategyV2__factory,
    IBaseStrategy,
    IERC20,
...
describe("Strategies Unit Test: Convex 2pool", () => {
    let accounts: AccountsFixture;

    before(async () => {
        await resetToBlockNumber(arbitrumBlock);
        ({ accounts } = await loadFixture(underlyingTokensFixture));
    });

    it("Should fail deploying strategy with underlying address 0", async () => {
        const ConvexSharedStrategy = new ConvexShared2poolStrategyV2__factory().connect(accounts.administrator);
        await expect(
        ...
        describe(`Asset: ${name}`, () => {
            ...
            describe(`Deployment: ${name}`, () => {
                it("Should deploy", async () => {
                    const convexStrat = await new ConvexShared2poolStrategyV2__factory()
            ...
            describe(`Functions: ${name}`, () => {
                ...
                beforeEach(async () => {
                    ...
                    let convexBoosterHelper = await new ConvexBoosterContractHelperV2__factory()
                        .connect(accounts.administrator)
                        .deploy(
                    ...
                    convexBoosterHelper = ConvexBoosterContractHelperV2__factory.connect(
                        helperProxy.address,
                        accounts.administrator
                    );

                    const convexStrategyImpl = await new ConvexShared2poolStrategyV2__factory()
                        .connect(accounts.administrator)
                        .deploy(
                            arbitrumConst.convex.Booster.address,
                            arbitrumConst.convex._2pool.boosterPoolId,
                    ...
                    convexContract = ConvexShared2poolStrategyV2__factory.connect(
                        convexStrategyProxy.address,
                        accounts.administrator
                    );
