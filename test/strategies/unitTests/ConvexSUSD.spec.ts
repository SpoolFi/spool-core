...
import {
    ...
    ConvexShared4poolStrategy__factory,
    IBaseStrategy,
    IERC20,
...
const swapData = [
    ...
    { slippage: 1, path: swapPathWeth },      // SNX
];
...
describe("Strategies Unit Test: Convex SUSD", () => {
    ...
    it("Should fail deploying Convex 4 Pool Strategy with underlying address 0", async () => {
        const ConvexShared4poolStrategy = await new ConvexShared4poolStrategy__factory().connect(accounts.administrator);
        await expect(
            ConvexShared4poolStrategy.deploy(
        ...
        describe(`Asset: ${name}`, () => {
            ...
            describe(`Deployment: ${name}`, () => {
                it("Should deploy", async () => {
                    const convexStrat = await new ConvexShared4poolStrategy__factory()
            ...
            describe(`Functions: ${name}`, () => {
                ...
                beforeEach(async () => {
                    ...
                    const convexStrategyImpl = await new ConvexShared4poolStrategy__factory()
                        .connect(accounts.administrator)
                        .deploy(
                            mainnetConst.convex.Booster.address,
                            mainnetConst.convex._sUSD.boosterPoolId,
                    ...
                    convexContract = ConvexShared4poolStrategy__factory.connect(
                        convexStrategyProxy.address,
                        accounts.administrator
                    );
