...
abstract contract SwapHelperBalancer {
    ...
    function _approveAndSwapBalancer(
        ...
        (BatchSwapStep[] memory swaps, IAsset[] memory assets) = _getBatchSwapsAndAssets(amount, swapData.path);
    ...
    function _swapBal(
        ...
        require(address(assets[swaps[0].assetInIndex]) == address(from), "SwapHelperBalancer:_swapBal:: from token incorrect.");
        require(address(assets[swaps[swaps.length-1].assetOutIndex]) == address(to), "SwapHelperBalancer:_swapBal:: to token incorrect.");
     ...
     */
    function _getBatchSwapsAndAssets(
        uint amount,
        ...
        // assert swap data
        uint numSwaps = pathBytes.toUint8(0);
        require(numSwaps > 0 && numSwaps <= MAX_SWAPS);
        
        // assert asset data
        uint startAssets = NUM_SWAPS_SIZE + (numSwaps * SWAPS_SIZE);
        uint assetsPathSize = pathBytes.length - startAssets; 
        uint numAssets = assetsPathSize  / ADDR_SIZE;
