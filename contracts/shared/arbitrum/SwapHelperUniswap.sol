...
import "../../external/sushiswap/interfaces/IV2SushiSwapRouter.sol";
...
/// @title Contains logic facilitating swapping using Uniswap on Arbitrum
abstract contract SwapHelperUniswap {
    ...
    /// @notice 
    Protocol private immutable protocol;
     ...
     */
    constructor(ISwapRouter02 _uniswapRouter, address _WETH, Protocol _protocol) {
    ...
    function _swapV2(
        ...
        if(protocol == Protocol.SUSHISWAP){
            uint[] memory amounts = 
                IV2SushiSwapRouter(address(uniswapRouter))
