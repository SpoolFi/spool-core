...
/** @notice Handle seeting zero value in a storage word as uint256 max value.
  *
  *  @dev
  ...
  *  so instead, if word is to be set to zero, we set it to uint256 max.
  *
  ...
  */
library Max256Bit {
    uint256 internal constant ZERO = type(uint256).max;

    function get(uint256 a) internal pure returns(uint256) {
        return (a == ZERO) ? 0 : a;
    }

    function set(uint256 a) internal pure returns(uint256){
        return (a == 0) ? ZERO : a;
    }
