...
abstract contract CurveStrategyBaseV3 is ProcessStrategy {
    ...
    constructor(
        ...
        require(_pool != address(0), "CurveStrategyBaseV3::constructor: Curve Pool address cannot be 0");
        require(address(_lpToken) != address(0), "CurveStrategyBaseV3::constructor: Token address cannot be 0");
    ...
    function _deposit(uint128 amount, uint256[] memory slippages) internal override returns(uint128) {
        ...
        require(isDeposit, "CurveStrategyBaseV3::_deposit: Withdraw slippage provided");
        ...
        newLp = _handleDeposit(newLp, (slippages.length > 1) ? slippages[1] : 0);
    ...
    function _withdraw(uint128 shares, uint256[] memory slippages) internal override returns(uint128) {
        (bool isDeposit, uint256 slippage) = _getSlippageAction(slippages[0]);
        require(!isDeposit, "CurveStrategyBaseV3::_withdraw: Deposit slippage provided");
        ...
        uint withdrawnLp = _handleWithdrawal(withdrawLp, (slippages.length > 1) ? slippages[1] : 0);

        // withdraw fTokens from vault
        uint256 undelyingBefore = underlying.balanceOf(address(this));
        lpToken.safeApprove(pool, withdrawnLp);
        
        _curveWithdrawal(withdrawnLp, slippage);
     ...
     * @param _lp amount of lp tokens to convert
     */
    function _lpToCoin(uint256 _lp) internal view returns (uint128) {
        if (_lp == 0)
            return 0;

        uint256 lp = _preLpToCoin(_lp);
     ...
     */
    function _preLpToCoin(uint256 _lp) internal virtual view returns(uint){
        return _lp;
    }
    ...
    function _handleDeposit(uint256 lp, uint256 slippages) internal virtual returns(uint);

    function _handleWithdrawal(uint256 lp, uint256 slippages) internal virtual returns(uint);
